...
class XmlResourceCompiler:
	...
	def GenerateClasses(self, resourceDocument):
		...
		topWindows = [
			e
			for e in resource.childNodes
			if e.nodeType == e.ELEMENT_NODE and e.tagName == "object" and not e.getAttribute("subclass")
		]
	...
	def CheckAssignVar(self, widget):
		if self.assignVariables:
			return True  # assign_var override mode
		assign_var = False
	...
	def GenerateMenuBarClass(self, windowClass, windowName, topWindow, vars):
		...
		for widget in topWindow.getElementsByTagName("object"):
			if not self.CheckAssignVar(widget):
				continue
		...
		for elem in topWindow.getElementsByTagName("XRCED"):
			...
			for event in events:
				if elem.parentNode is topWindow:
					...
					widgetClass = re.sub("^wx", "", widgetClass)
					widgetName = widget.getAttribute("name")
					if not widgetName or not widgetClass:
						continue
					eventObject = None
					if widgetClass == "MenuItem" and windowClass != "MenuBar":
						...
						if widgetName in vars:
							eventObject = "self.%s" % widgetName
					else:
						eventHandler = "On%s_%s" % (event[4:].capitalize(), widgetName)
						if widgetName in vars:
							eventObject = "self.%s" % widgetName
	...
	def NodeContainsFilename(self, node):
		...
		if (
			parent.__class__ != minidom.Document
			and parent.getAttribute("class") == "wxBitmapButton"
			and (node.nodeName == "focus" or node.nodeName == "disabled" or node.nodeName == "selected")
		):
			return True
	...
	def ReplaceFilenamesInXRC(self, node, files, resourcePath):
		...
		if node is None:
			return
		if node.nodeType != minidom.Document.ELEMENT_NODE:
			return
		...
		for n in node.childNodes:
			if containsFilename and (
				n.nodeType == minidom.Document.TEXT_NODE or n.nodeType == minidom.Document.CDATA_SECTION_NODE
			):
	...
	def FindStringsInNode(self, parent):
		...
		for child in parent.childNodes:
			if (
				...
				(child.nodeType == child.TEXT_NODE or child.nodeType == child.CDATA_SECTION_NODE)
				and
				# ...it is textnode...
				(
					parent.tagName == "label"
					or (parent.tagName == "value" and not is_number(child.nodeValue))
					or parent.tagName == "help"
					or parent.tagName == "longhelp"
					or parent.tagName == "tooltip"
					or parent.tagName == "htmlcode"
					or parent.tagName == "title"
					or parent.tagName == "item"
				)
			):
