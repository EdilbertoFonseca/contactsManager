...
from .varsConfig import ADDON_NAME, ADDON_PATH, EMAIL_REGEX, IS64, ourAddon
...
class AddEditRecDialog(wx.Dialog):
	def __init__(
		...
		wx.Dialog.__init__(
			self,
			parent,
			title=_("{} Record in Contacts Manager for NVDA").format(title),
			size=(WIDTH, HEIGHT),
		)

		# Check and apply settings.
		self.formatCellPhone = cast(Any, config.conf[ourAddon.name])["formatCellPhone"]
		self.formatLandline = cast(Any, config.conf[ourAddon.name])["formatLandline"]
