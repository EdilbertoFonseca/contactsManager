...
class TimeCtrl(BaseMaskedTextCtrl):
	...
	def __init__(
		...
		**kwargs,
		...
		self.Bind(
			wx.EVT_TEXT, self.__OnTextChange, self,
			...
			if key == "format":
				...
				if require24hr and not self.__fmt24hr:
					raise AttributeError(
						'"%s" is an unsupported time format for the current locale' % param_value,
					)
		...
		* mxDateTimeDelta
		...
		if isinstance(value, str):
			...
			if not valid:
				...
				try:
					if wxdt.Format("%p") not in ("AM", "PM") and checkTime in (5, 8):
						...
						raise ValueError(
							'cannot convert string "%s" to valid time for the current locale; please use 24hr time instead'
							% value,
						)
					else:
			...
			if isinstance(value, wx.DateTime):
				...
				if accept_mx:
					error = (
						"GetWxDateTime requires wxDateTime, mxDateTime or parsable time string, passed %s"
						% repr(value)
					)
				else:
					error = "GetWxDateTime requires wxDateTime or parsable time string, passed %s" % repr(
						value,
					)
				##                dbg(indent=0, suspend=0)
	...
	def _toGUI(self, wxdt):
		...
		if self.__fmt24hr:
			if self.__displaySeconds:
				strval = wxdt.Format("%H:%M:%S")
			else:
				strval = wxdt.Format("%H:%M")
		else:
			if self.__displaySeconds:
				strval = wxdt.Format("%I:%M:%S %p")
			else:
				strval = wxdt.Format("%I:%M %p")
