...
class MaskedComboBoxEventHandler(wx.EvtHandler):
	...
	def __init__(self, combobox):
		...
		self.Bind(
			wx.EVT_KEY_DOWN, self.combobox._OnKeyDownInComboBox,
		...
		## (next in evt chain)
		self.Bind(
			wx.EVT_COMBOBOX, self.combobox._OnDropdownSelect,
...
class BaseMaskedComboBox(wx.ComboBox, MaskedEditMixin):
	...
	def SetValue(self, value):
		...
		if (
			len(value) < self._masklength  # value shorter than control
			and (self._isFloat or self._isInt)  # and it's a numeric control
			and self._ctrl_constraints._alignRight
	...
	def Append(self, choice, clientData=None):
		...
		if self._mask:
			if not isinstance(choice, str):
				...
				raise ValueError(
					'%s: "%s" is not a valid value for the control as specified.' % (str(self._index), choice),
				)
			...
			if not self.IsValid(choice) and (
				not self._ctrl_constraints.IsEmpty(choice)
				or (self._ctrl_constraints.IsEmpty(choice) and self._ctrl_constraints._validRequired)
			):
				raise ValueError(
					'"%s" is not a valid value for the control "%s" as specified.' % (choice, self.name),
				)
	...
	def _OnReturn(self, event):
		...
		if (
			self.GetCurrentSelection() == -1
			and self.GetValue().lower().strip() in self._ctrl_constraints._compareChoices
		):
