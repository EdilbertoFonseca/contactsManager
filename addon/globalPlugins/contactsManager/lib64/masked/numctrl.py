...
class NumCtrl(BaseMaskedTextCtrl, NumCtrlAccessorsMixin):
		...
		**kwargs,
		...
		if not isinstance(init_args["integerWidth"], int):
			raise AttributeError(
				"invalid integerWidth (%s) specified; expected integer" % repr(init_args["integerWidth"]),
			)
		elif init_args["integerWidth"] < 1:
			raise AttributeError(
				"invalid integerWidth (%s) specified; must be > 0" % repr(init_args["integerWidth"]),
			)
		...
		if "fractionWidth" in init_args:
			if not isinstance(init_args["fractionWidth"], int):
				raise AttributeError(
					"invalid fractionWidth (%s) specified; expected integer" % repr(self._fractionWidth),
				)
			elif init_args["fractionWidth"] < 0:
				raise AttributeError(
					"invalid fractionWidth (%s) specified; must be >= 0" % repr(init_args["fractionWidth"]),
				)
	...
	def SetParameters(self, **kwargs):
		...
		if (
			("integerWidth" in kwargs and kwargs["integerWidth"] != self._integerWidth)
			...
			if "fractionWidth" in kwargs:
				if not isinstance(kwargs["fractionWidth"], int):
					raise AttributeError(
						"invalid fractionWidth (%s) specified; expected integer"
						% repr(kwargs["fractionWidth"]),
					)
				elif kwargs["fractionWidth"] < 0:
					raise AttributeError(
						"invalid fractionWidth (%s) specified; must be >= 0" % repr(kwargs["fractionWidth"]),
					)
				else:
			...
			if "integerWidth" in kwargs:
				if not isinstance(kwargs["integerWidth"], int):
					##                    dbg(indent=0)
					raise AttributeError(
						"invalid integerWidth (%s) specified; expected integer" % repr(kwargs["integerWidth"]),
					)
				elif kwargs["integerWidth"] < 0:
					##                    dbg(indent=0)
					raise AttributeError(
						"invalid integerWidth (%s) specified; must be > 0" % repr(kwargs["integerWidth"]),
					)
				else:
		...
		if "min" in kwargs:
			min = kwargs["min"]
			if self._max is None or min is None or (self._max is not None and self._max >= min):
				##                dbg('examining min')
				if min is not None:
					try:
		...
		if "max" in kwargs:
			max = kwargs["max"]
			if self._min is None or max is None or (self._min is not None and self._min <= max):
				##                dbg('examining max')
				if max is not None:
					try:
		...
		if (
			"decimalChar" in kwargs
			and self._decimalChar != old_decimalchar
			and text.find(old_decimalchar) != -1
		):
			text = old_numvalue
	...
	def _SetValue(self, value):
		...
		if replacement is not None:
			...
			sel_start = BaseMaskedTextCtrl.GetValue(self).find(
				str(abs(replacement)),
		...
		if self._groupDigits:
			...
			if key == wx.WXK_BACK:
				...
				# elif previous char is group char, select to previous digit
					...
					and value[sel_start - 1 : sel_start] == self._groupChar
				):
				...
				if (
					sel_to < len(self._mask) - 2 + (1 * self._useParens)
					and sel_start == sel_to
	...
	def _toGUI(self, value, apply_limits=True):
		...
		if len(s) > (adjustwidth - self._groupSpace):
			##            dbg(indent=0)
			raise ValueError(
				"value %s exceeds the integer width of the control (%d)" % (s, self._integerWidth),
			)
	...
	def _Undo(self, value=None, prev=None):
		...
		if not self._groupDigits:
			ignore, (new_sel_start, new_sel_to) = BaseMaskedTextCtrl._Undo(
				self, value, prev, just_return_results=True,
			)
			self._SetValue(prev)
		...
		ignore, (new_sel_start, new_sel_to) = BaseMaskedTextCtrl._Undo(
			self, adjvalue, adjprev, just_return_results=True,
		)
