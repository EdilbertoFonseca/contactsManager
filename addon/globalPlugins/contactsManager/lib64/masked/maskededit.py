...
class Field:
	...
	def _ValidateParameters(self, **kwargs):
		...
		if self._groupdigits and self._groupChar == self._decimalChar:
			##            dbg(indent=0, suspend=0)
			ae = AttributeError(
				"groupChar '%s' cannot be the same as decimalChar '%s'" % (self._groupChar, self._decimalChar),
			)
		...
		if "validRegex" in kwargs:  # (set/changed)
			if self._validRegex:
				try:
					...
					##                    dbg(indent=0, suspend=0)
					raise TypeError(
						'%s: validRegex "%s" not a legal regular expression'
						% (str(self._index), self._validRegex),
					)
			else:
				self._filter = None
		...
		if "validRange" in kwargs:  # (set/changed)
			...
			if self._validRange:
				if (
					not isinstance(self._validRange, tuple)
					or len(self._validRange) != 2
					or self._validRange[0] > self._validRange[1]
				):
					##                    dbg(indent=0, suspend=0)
					raise TypeError(
		...
		if "choices" in kwargs or (
			len(self._choices) and len(self._choices[0]) != len(self._mask)
		):  # (set/changed)
			self._hasList = False
				...
				if hasattr(self, "_template"):
					...
					for choice in self._choices:
						...
						if not self.IsValid(choice):
							##                            dbg(indent=0, suspend=0)
							ve = ValueError(
								'%s: "%s" is not a valid value for the control as specified.'
								% (str(self._index), choice),
							)
							ve.value = choice
							raise ve
				self._hasList = True
		...
		if "fillChar" in kwargs and len(self._choices) > 0:
			if kwargs["fillChar"] != " ":
				...
				self._choices = [
					choice.replace(self._old_fillChar, self._fillChar) for choice in self._choices
				]
...
class MaskedEditMixin:
	...
	def SetCtrlParameters(self, **kwargs):
		...
		if autoformat != self._autoformat and autoformat in masktags:
			...
			for param, value in masktags[self._autoformat].items():
				if param == "mask":
					continue  # (must be present; already accounted for)
				constraint_kwargs[param] = value

		elif autoformat and autoformat not in masktags:
		...
		for key in (
			...
			if key in ctrl_kwargs:
				if isinstance(ctrl_kwargs[key], str):
					...
					if c.Get() == (-1, -1, -1):
						raise TypeError(
							"%s not a legal color specification for %s" % (repr(ctrl_kwargs[key]), key),
						)
					else:
		...
		if self._isDate and self._autoformat:
			...
			if self._autoformat.find("MDDY") != -1:
				self._datestyle = "MDY"
			elif self._autoformat.find("YMMD") != -1:
				self._datestyle = "YMD"
			elif self._autoformat.find("YMMMD") != -1:
				self._datestyle = "YMD"
			elif self._autoformat.find("DMMY") != -1:
				self._datestyle = "DMY"
			elif self._autoformat.find("DMMMY") != -1:
				self._datestyle = "DMY"
	...
	def _calcFieldExtents(self):
		...
		if self._mask:
			...
			if self._isFloat:
				...
				if "R" not in formatcodes:
					formatcodes += "R"
				self._fields[0]._SetParameters(
					index=0,
					extent=(start, self._decimalpos),
	...
	def _calcTemplate(self, reset_fillchar, reset_default):
		...
		if self.controlInitialized:
			curvalue = list(self._GetValue())
		else:
			curvalue = None

		if hasattr(self, "_fillChar"):
			old_fillchars = self._fillChar
		else:
			old_fillchars = None
		...
		if default_set:
			...
			if not self.IsEmpty(self._defaultValue) and not self.IsValid(self._defaultValue):
				####                dbg(indent=0)
				ve = ValueError(
					'Default value of "%s" is not a valid value for control "%s"'
					% (self._defaultValue, self.name),
				)
	...
	def _validateChoices(self):
		...
		for field in self._fields.values():
			if field._choices:
				index = field._index
				if (
					len(self._field_indices) == 1
					and index == 0
					and field._choices == self._ctrl_constraints._choices
				):
				...
				for choice in field._choices:
					...
					if not valid_paste:
						####                        dbg(indent=0)
						ve = ValueError(
							'"%s" could not be entered into field %d of control "%s"'
							% (choice, index, self.name),
						)
						ve.value = choice
						ve.index = index
						raise ve
						...
						####                        dbg(indent=0)
						ve = ValueError(
							'"%s" will not fit into field %d of control "%s"' % (choice, index, self.name),
						)
						ve.value = choice
						ve.index = index
						raise ve
		...
		if self._isFloat or self._isInt:
			if self.controlInitialized:
				...
				if len(value) < len(self._ctrl_constraints._mask):
					...
					if (
						self._useParens
						and len(newvalue) < len(self._ctrl_constraints._mask)
						and newvalue.find("(") == -1
					):
						newvalue += " "
					if (
			...
			if wx.WXK_UP not in self._keyhandlers:
				self._SetKeycodeHandler(
					wx.WXK_UP, self._OnUpNumeric,
			...
			if (
				ord(self._decimalChar) not in self._keyhandlers
				or self._keyhandlers[ord(self._decimalChar)] != self._OnDecimalPoint
			):
				self._SetKeyHandler(self._decimalChar, self._OnDecimalPoint)

			if (
				ord(self._shiftDecimalChar) not in self._keyhandlers
				or self._keyhandlers[ord(self._shiftDecimalChar)] != self._OnChangeField
			):
				self._SetKeyHandler(
					self._shiftDecimalChar, self._OnChangeField,
				)  # (Shift-'.' == '>' on US keyboards)

			# Allow selective insert of groupchar in numbers:
			if (
				ord(self._fields[0]._groupChar) not in self._keyhandlers
				or self._keyhandlers[ord(self._fields[0]._groupChar)] != self._OnGroupChar
			):
	...
	def _setFont(self):
		...
		if not self._useFixedWidthFont:
			...
			if "wxMac" in wx.PlatformInfo and self.GetWindowVariant() == wx.WINDOW_VARIANT_MINI:
				points -= 1
	...
	def _OnArrow(self, event):
		...
		if keycode in (wx.WXK_RIGHT, wx.WXK_DOWN, wx.WXK_NUMPAD_RIGHT, wx.WXK_NUMPAD_DOWN):
			if (not self._isTemplateChar(pos) and pos + 1 > entry_end) or (
				self._isTemplateChar(pos) and pos >= entry_end
			):
			...
			keycode in (wx.WXK_LEFT, wx.WXK_UP, wx.WXK_NUMPAD_LEFT, wx.WXK_NUMPAD_UP)
			and sel_start == sel_to
			and pos > 0
			and self._isTemplateChar(pos - 1)
		):
			##            dbg('adjusting field')
			self._AdjustField(pos)
		...
		if event.ShiftDown() and keycode in (wx.WXK_UP, wx.WXK_DOWN, wx.WXK_NUMPAD_UP, wx.WXK_NUMPAD_DOWN):
			...
			if (
				keycode in (wx.WXK_UP, wx.WXK_LEFT, wx.WXK_NUMPAD_UP, wx.WXK_NUMPAD_LEFT)
				and sel_start != 0
				and self._isTemplateChar(sel_start - 1)
				and sel_start != self._masklength
				and not self._signOk
				and not self._useParens
			):
				...
				keycode in (wx.WXK_DOWN, wx.WXK_RIGHT, wx.WXK_NUMPAD_DOWN, wx.WXK_NUMPAD_RIGHT)
			...
			if (sel_to == self._fields[0]._extent[0] and keycode in (wx.WXK_LEFT, wx.WXK_NUMPAD_LEFT)) or (
				sel_to == self._masklength and keycode in (wx.WXK_RIGHT, wx.WXK_NUMPAD_RIGHT)
			):
	...
	def _OnCtrl_S(self, event):
		...
		if self._demo:
			print(
				'MaskedEditMixin.GetValue()       = "%s"\nMaskedEditMixin.GetPlainValue() = "%s"'
				% (self.GetValue(), self.GetPlainValue()),
			)
	...
	def _OnErase(self, event=None, just_return_value=False):
		...
		if (
			(sel_to == 0 and key == wx.WXK_BACK)
			or (self._signOk and sel_to == 1 and value[0] == " " and key == wx.WXK_BACK)
			or (
				sel_to == self._masklength
				and sel_start == sel_to
				and key in (wx.WXK_DELETE, wx.WXK_NUMPAD_DELETE)
				and not field._insertRight
			)
			or (
				self._signOk
				and self._useParens
		...
		if (
			field._insertRight  # an insert-right field
			...
			for i in range(start, end - len(newfield)):
				if field._padZero:
					...
					and i == 1
					and (
						(self._useParens and newfield.find("(") == -1)
			...
			if field._allowInsert and sel_start >= start:
				...
				if sel_start == sel_to:
					...
					if field._padZero and not value[start:sel_to].replace("0", "").replace(" ", "").replace(
						field._fillChar, "",
					):
						# preceding chars (if any) are zeros, blanks or fillchar; new char should be 0:
						newchar = "0"
					else:
						newchar = self._template[
							sel_to
					...
					if self._isTemplateChar(sel_to):
						if (
							sel_to == 0 and self._signOk and value[sel_to] == "-"
							...
							self._signOk
							and self._useParens
							and (value[sel_to] == ")" or value[sel_to] == "(")
						):
		...
		if not event.ControlDown():
			...
			if field != field_end or sel_to >= end_of_input:
				...
				if sel_to == edit_end and field._index < self._field_indices[-1]:
					edit_start, edit_end = self._FindFieldExtent(
						self._findNextEntry(edit_end),
	...
	def _OnHome(self, event):
		...
		elif event.ControlDown() and (not event.ShiftDown() or (event.ShiftDown() and sel_start > 0)):
			if len(self._field_indices) > 1:
				field = self._FindField(sel_start)
				start, ignore = field._extent
				if (
					sel_start == start and field._index != self._field_indices[0]
	...
	def _findNextEntry(self, pos, adjustInsert=True):
		...
		if (
			self._isTemplateChar(pos) or pos in self._explicit_field_boundaries
	...
	def _OnAutoCompleteField(self, event):
		...
		if field._hasList:
			...
			match_index, partial_match = self._autoComplete(
				...
				current_index=field._autoCompleteIndex,
			)
			if match_index is None and (
				keycode
				in self._autoCompleteKeycodes
				+ [wx.WXK_PAGEUP, wx.WXK_PAGEDOWN, wx.WXK_NUMPAD_PAGEUP, wx.WXK_NUMPAD_PAGEDOWN]
				or (
					keycode in [wx.WXK_UP, wx.WXK_DOWN, wx.WXK_NUMPAD_UP, wx.WXK_NUMPAD_DOWN]
					and event.ShiftDown()
				)
			):
		...
		if value in choices:
			...
			if current_index is None:
				...
				##                    dbg('range(current_index+1 (%d), len(choices) (%d)) + range(%d):' % (current_index+1, len(choices), current_index+1), indices)
					...
					indices = range(current_index - 1, -1, -1) + range(
						len(choices) - 1, current_index - 1, -1,
					)
	...
	def _adjustPos(self, pos, key):
		...
		if self._signOk and (
			(pos == self._signpos and key in (ord("-"), ord("+"), ord(" ")))
		...
		if key not in self._nav:
			...
			if field._insertRight:  # if allow right-insert
				...
				if pos == end:  # if cursor at right edge of field
					...
					if (
						len(slice) == field_len
						and field._moveOnFieldFull
						and (
							not field._stopFieldChangeIfInvalid
							or field._stopFieldChangeIfInvalid
							and field.IsValid(slice)
						)
					):
			...
			# If they are, move them to the next valid entry position
				...
				if (
					...
					and (not self._signOk or (self._signOk and field._index == 0 and pos > 0))
				) or (field._stopFieldChangeIfInvalid and not field.IsValid(self._GetValue()[start:end])):
	...
	def _adjustInt(self, candidate=None):
		...
		if self._signOk and (
			(self._useParens and intStr.find("(") == -1) or (not self._useParens and intStr.find("-") == -1)
		):
			intStr = " " + intStr
			if self._useParens:
	...
	def _adjustDate(self, candidate=None, fixcentury=False, force4digit_year=False):
		...
		year = _getYear(text, self._datestyle).replace(
			self._fields[year_field]._fillChar, "",
	...
	def _isCharAllowed(self, char, pos, checkRegex=False, allowAutoSelect=True, ignoreInsertRight=False):
		...
		if (field._insertRight or self._ctrl_constraints._insertRight) and not ignoreInsertRight:
			...
			if (sel_start, sel_to) == field._extent or (
				pos == end
				and (
					(input_len < field_len)
					or (field_len == 1 and input_len == field_len and field._isInt and value[end - 1] == "0")
				)
			):
				pos = end - 1
		...
		if self._isMaskChar(pos):
			...
			if approved and checkRegex:
				...
				if right_insert:
					...
					at = pos
				if allowAutoSelect:
					newvalue, ignore, ignore, ignore, ignore = self._insertKey(
						char, at, sel_start, sel_to, value, allowAutoSelect=True,
					)
				else:
	...
	def _getAbsValue(self, candidate=None):
		...
		if self._isInt:
			...
			try:
				groupchar = self._fields[0]._groupChar
				value = float(
					text.replace(groupchar, "")
					.replace(self._decimalChar, ".")
					.replace("(", "-")
					.replace(")", "")
					.replace(" ", ""),
				)
	...
	def _insertKey(self, char, pos, sel_start, sel_to, value, allowAutoSelect=False):
		...
		if sel_start != sel_to and sel_to >= sel_start + 2:
			field = self._FindField(sel_start)
			if (
				field._insertRight  # if right-insert
				and field._allowInsert  # and allow insert at any point in field
				and field == self._FindField(sel_to)
		...
		if (
			field._insertRight  # field allows right insert
			and (
		...
		if not newtext:
			...
			if (
				field._insertRight  # if insert-right field (but we didn't start at right edge)
			...
			if field._autoSelect:
				...
				if match_index is not None and partial_match:
					...
					if self._ctrl_constraints._insertRight:
						...
						newpos = self._masklength - (
							len(self._ctrl_constraints._choices[match_index].strip())
							- len(matched_str.strip())
						)
	...
	def _fixSelection(self):
		...
		if (
			sel_start == 0
			and sel_to >= len(self._mask)  # (can be greater in numeric controls because of reserved space)
	...
	def _validatePaste(self, paste_text, sel_start, sel_to, raise_on_invalid=False):
		...
		while valid_paste and i < length_considered and replace_to < self._masklength:
			...
			if not field._compareNoCase:
				if field._forceupper:
					char = char.upper()
				elif field._forcelower:
					char = char.lower()
			...
			if not self._isTemplateChar(replace_to) and self._isCharAllowed(
				...
				i += 1
				replace_to += 1
			elif char == self._template[replace_to] or (
				self._signOk
				and (
	...
	def _Paste(self, value=None, raise_on_invalid=False, just_return_value=False):
		...
		if paste_text is not None:
			...
			if (
				field._allowInsert
				and sel_to <= edit_end
				and (sel_start + len(paste_text) < edit_end or field._insertRight)
			):
				if field._insertRight:
			...
			if (
				len(paste_text) > sel_to - sel_start
			...
			if self._signOk:
				...
				if paste_signpos != -1 and (
					sel_start <= signpos or (field._insertRight and sel_start - len(paste_text) <= signpos)
				):
					signed = True
				else:
					signed = False
			...
			try:
				valid_paste, replacement_text, replace_to = self._validatePaste(
					paste_text, sel_start, sel_to, raise_on_invalid,
				)
			except:
				...
				if new_text:
					if self._useParens:
						new_text = (
							new_text[:signpos]
							+ "("
							+ new_text[signpos + 1 : right_signpos]
							+ ")"
							+ new_text[right_signpos + 1 :]
						)
					else:
		...
		if prev is None:
			...
			if (sel_start, sel_to) != self._prevSelection:
				...
				if (
					self._signOk
					and sel_start < self._masklength
					and (prev[sel_start] in ("-", "(", ")") or value[sel_start] in ("-", "(", ")"))
				):
					...
					value[sel_start:sel_to] == field._groupChar or prev[sel_start:sel_to] == field._groupChar
				):
					...
					##                            dbg('setting selection to', (sel_start, sel_to))
						...
						sel_to > prev_sel_to  # calculated select past last selection
						and prev_sel_to < len(self._template)  # and prev_sel_to not at end of control
						and sel_to == len(self._template)
				...
				while sel_to - 1 > 0 and sel_to > sel_start and value[sel_to - 1 :] == prev[sel_to - 1 :]:
					sel_to -= 1
				while (
					sel_start + 1 < self._masklength
					and sel_start < sel_to
...
Try entering nonsensical or partial values in validated fields to see what happens (use ctrl-s to test the valid status)."""
		...
		for control in controls:
			self.sizer.Add(
				wx.StaticText(self.panel, -1, control[0]), row=rowcount, col=0, border=5, flag=wx.ALL,
			)
			self.sizer.Add(
				wx.StaticText(self.panel, -1, control[1]), row=rowcount, col=1, border=5, flag=wx.ALL,
			)
			self.sizer.Add(
				wx.StaticText(self.panel, -1, control[3]), row=rowcount, col=2, border=5, flag=wx.ALL,
			)
			self.sizer.Add(
				wx.StaticText(self.panel, -1, control[4][:20]), row=rowcount, col=3, border=5, flag=wx.ALL,
			)

			if control in controls[:]:  # -2]:
				newControl = MaskedTextCtrl(
					self.panel,
					-1,
					"",
					mask=control[1],
				...
				if control[6]:
					newControl.SetCtrlParameters(choiceRequired=True)
			else:
				newControl = MaskedComboBox(
					self.panel,
					-1,
					"",
					choices=control[7],
...
class __test2(wx.Frame):
	def __init__(self, parent, id, caption):
		wx.Frame.__init__(
			self, parent, id, "MaskedEdit control 0.0.7 Demo Page #2 -- AutoFormats", size=(550, 600),
		)
		from wx.lib.rcsizer import RowColSizer
...
To see a great example of validations in action, try entering a bad email address, then tab out."""
		...
		for control in controls:
			...
			if control in controls[:-1]:
				...
				self.sizer.Add(
					IpAddrCtrl(self.panel, -1, "", demo=True),
					row=rowcount,
					col=2,
					flag=wx.ALL,
					border=5,
